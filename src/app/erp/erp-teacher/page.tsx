  ...
  return (
    <>
      <PageHeader
        eyebrow="ERP"
        title="Teacher Login"
        subtitle="Access your teacher portal for attendance, marks, and resources."
        crumbs={[{ label: "Home", href: "/" }, { label: "ERP" }, { label: "Teacher Login" }]}
      />
      <Section className="bg-beige-200">
        <div className="max-w-md mx-auto">
          <div className="card !p-8">
            ...
            <h2 className="font-display text-2xl text-navy-900 text-center mb-1">Teacher Portal</h2>
            <p className="text-sm text-navy-600 text-center mb-6">Sign in with your teacher credentials.</p>
            {submitted ? (
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-navy-800 mb-1">Teacher ID</label>
                  <input required type="text" value={form.id} onChange={(e) => setForm({ ...form, id: e.target.value })} className="w-full border border-navy-200 px-3 py-2 text-sm focus:border-caramel-400 focus:outline-none" placeholder="Enter your teacher ID" />
                </div>
                <div>
